...
public class RatingsService {
	...
	public void submitRatings(Rating rating){
		...
		Optional<Doctor> savedDoctor = doctorRepository.findById(rating.getDoctorId());
		if(savedDoctor.isPresent()){
			savedDoctor.get().setRating(rating.getRating().doubleValue());
			doctorRepository.save(savedDoctor.get());
		}else {
